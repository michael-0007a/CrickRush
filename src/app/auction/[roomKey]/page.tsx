...
export default function LiveAuctionPage() {
  ...
  useEffect(() => {
    const loadParticipants = async () => {
      ...
      try {
        console.log('🔍 Loading participants for room:', room.id);
        ...
        if (error) {
          ...
          console.log('✅ Loaded participants:', participantData?.length || 0);
        ...
        try {
          ...
          if (sessionError) {
            ...
            if (sessionError.message?.includes('Refresh Token') || sessionError.message?.includes('Invalid')) {
              console.log('Invalid or expired session, redirecting to auth');
              router.push('/auth');
              return;
            }
          }
  ...
  useEffect(() => {
    ...
    if (!myParticipant && !isAuctioneer) {
      console.log('User needs to join - but staying on this page to avoid infinite loop');
    }
  ...
  const joinWithTeam = async () => {
    ...
    try {
      console.log('🚀 Joining auction with team:', selectedTeam);
      ...
      console.log('✅ Successfully joined auction:', insertedParticipant);

      setShowTeamSelector(false);
      alert('Successfully joined the auction!');

      // Don't reload - just trigger a manual refresh of participants
  ...
  const placeBid = async (amount: number) => {
    ...
    try {
      ...
      console.log('Bid placed, SQL trigger should add 10 seconds automatically');
  ...
  const sellPlayer = async () => {
    ...
    try {
      ...
      // Update participant budget and force immediate refresh
      ...
      console.log('💰 Player sold - updating UI immediately');

      // Force immediate refresh of participants to show updated budget
      ...
      if (updatedParticipants) {
        ...
        setParticipants(mappedParticipants);
        console.log('✅ Participants updated with new budget:', mappedParticipants);
      }

      // Create the new player object for squad
      const newSquadPlayer = {
        ...
        player: {
          ...
          role: auctionState.current_player.role,
          country: auctionState.current_player.country,
          base_price: auctionState.current_player.base_price
        }
      };

      // If this is the current user's purchase, update their squad and budget immediately
      if (leadingParticipant.user_id === user?.id) {
        console.log('🎯 This is my purchase - updating my data immediately');
        addPlayer(newSquadPlayer);
      }

      alert(`Player sold to ${leadingParticipant.team_short_name} for ${formatCurrency(auctionState.current_bid)}!`);

      // Move to next player immediately - removed delay
      moveToNextPlayer();
  ...
  const endAuction = async () => {
    ...
    try {
      console.log('Ending auction with soft delete approach for room ID:', room.id);

      // Instead of using 'deleted', use 'completed' status which is allowed
      ...
      console.log('Auction marked as completed successfully from live page');
        ...
        <div style={{
          ...
          <div className="card card-lg" style={{ maxWidth: '64rem', width: '90%', maxHeight: '80vh', overflow: 'auto' }}>
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold mb-2">Choose Your Team</h2>
              <p style={{ color: 'var(--text-muted)' }}>
                Select an IPL franchise to represent in this auction
              </p>
            </div>
